...
OPENAI_GROUNDED_OBSERVATION_PROMPT = (
    ...
    "Do not translate, normalize, reconstruct, or complete unclear legends. "
    "Copy only text you can actually see. A date_like value may contain only "
    "visible numerals that resemble a date; denomination_mark may contain only "
    "a visible numeric or unit mark. Describe a portrait only when visibly "
    "defensible and describe appearance, not identity. Motifs must be visible "
    "objects or design elements. Missing evidence is preferred to guessed "
    "evidence. Return only the requested structured observation."
)
