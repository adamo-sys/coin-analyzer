...
class ConfirmedObservationTests(unittest.TestCase):
    ...
    def test_successful_detection_save_records_once_and_failed_or_invalid_save_records_nothing(self):
        ...
        invalid = self.make_gui(should_save=True)
        invalid.acquisition_controls = {"values": lambda: {"purchase_price": "-1"}}
    ...
    def test_incomplete_identity_save_records_observation_after_success(self):
        incomplete = self.make_gui(should_save=True, country="")
        with patch("coin_collection_gui.messagebox.showinfo"):
            incomplete.save_to_collection()
        self.assertEqual(1, incomplete.app.calls)
        incomplete.record_detection_observation_after_save.assert_called_once()
        self.assertEqual("", incomplete.record_detection_observation_after_save.call_args.args[0])
