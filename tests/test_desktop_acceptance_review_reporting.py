...
def _report(source: object, payload: object):
    ...
    return build_review_progress_report(source, execution), execution
...
def _data(report, source: object, execution: ReviewExecutionRecord):
    return validated_review_progress_dict(report, source, execution)
...
class DesktopAcceptanceReviewReportingTests(unittest.TestCase):
    ...
    def test_equivalent_input_order_produces_byte_identical_json(self) -> None:
        ...
        self.assertEqual(
            normalized_review_progress_json(report_a, source_a, execution_a),
            normalized_review_progress_json(report_b, source_b, execution_b),
        )
        self.assertEqual(
            render_review_progress_report(report_a, source_a, execution_a),
            render_review_progress_report(report_b, source_b, execution_b),
        )
        self.assertEqual(
    ...
    def test_human_report_reflects_the_same_derived_state_as_json(self) -> None:
        ...
        payload = _unresolved_payload(source)
        report, execution = _report(source, payload)
        machine = json.loads(normalized_review_progress_json(report, source, execution))
        human = render_review_progress_report(report, source, execution)
    ...
    def test_malformed_inputs_and_unchecked_report_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(DesktopAcceptanceReviewError, "case roster mismatch"):
            build_review_progress_report(source, unchecked_execution)

        report = build_review_progress_report(source, execution)
        ...
        with self.assertRaisesRegex(DesktopAcceptanceReviewError, "does not match"):
            validated_review_progress_dict(unchecked_report, source, execution)
        ...
        with self.assertRaisesRegex(DesktopAcceptanceAuthoringError, "missing fields"):
            build_review_progress_report(malformed_source, execution)

    def test_authoring_contract_is_fully_revalidated_before_reporting(self) -> None:
        source = authoring_state()
        execution = _validated(source, _ready_payload(source))
        mutations = (
        ...
        for mutate in mutations:
            with self.subTest(mutate=mutate):
                ...
                with self.assertRaises(DesktopAcceptanceAuthoringError):
                    build_review_progress_report(malformed, execution)

        with self.assertRaises(DesktopAcceptanceAuthoringError):
            build_review_progress_report([], execution)
        ...
        with self.assertRaisesRegex(DesktopAcceptanceReviewError, "duplicate authoring case"):
            build_review_progress_report(duplicate, execution)
    ...
    def test_reporting_does_not_mutate_inputs(self) -> None:
        ...
        report = build_review_progress_report(source, execution)
        normalized_review_progress_json(report, source, execution)
        render_review_progress_report(report, source, execution)
