...
class CoinCollectionGUI:
    ...
    def create_menu_bar(self):
        ...
        # Session & Data
        session_data_menu = tk.Menu(tools_menu, tearoff=0)
        tools_menu.add_cascade(label="Session & Data", menu=session_data_menu)
        session_data_menu.add_command(label="Load Collection Context", command=self.load_collection_context)
        session_data_menu.add_command(label="Clear Session Context", command=self.clear_session_context)
        session_data_menu.add_separator()
        session_data_menu.add_command(label="Save Session State", command=self.save_session_state)
        session_data_menu.add_command(label="Load Session State", command=self.load_session_state)
        session_data_menu.add_command(label="Clear Session State", command=self.clear_saved_session_state)
        session_data_menu.add_command(label="Export Session State", command=self.export_session_state)
        session_data_menu.add_command(label="Import Session State", command=self.import_session_state)
        session_data_menu.add_separator()
        session_data_menu.add_command(label="Create Backup Package", command=self.create_backup_package)
        session_data_menu.add_command(label="List Backups", command=self.list_backup_packages)
        session_data_menu.add_command(label="Restore Backup", command=self.restore_backup_package)
        session_data_menu.add_command(label="Create Snapshot", command=self.create_collection_snapshot)

        # Collection Intelligence
        collection_intelligence_menu = tk.Menu(tools_menu, tearoff=0)
        tools_menu.add_cascade(label="Collection Intelligence", menu=collection_intelligence_menu)
        ...
        collection_intelligence_menu.add_command(label="Acquisition Strategy", command=self.open_acquisition_strategy)
        collection_intelligence_menu.add_command(label="Ask My Collection", command=self.open_ask_my_collection)
        collection_intelligence_menu.add_command(label="Collection Assistant", command=self.open_collection_assistant)
        collection_intelligence_menu.add_command(label="Portfolio Analytics", command=self.open_portfolio_performance)
        collection_intelligence_menu.add_command(label="Numista Intelligence", command=self.open_numista_intelligence)

        # OCR & AI
        ocr_ai_menu = tk.Menu(tools_menu, tearoff=0)
        tools_menu.add_cascade(label="OCR & AI", menu=ocr_ai_menu)
        ocr_ai_menu.add_command(label="OCR Experiment", command=self.open_ocr_experiment)
        ocr_ai_menu.add_command(label="OCR-Assisted Identification", command=self.open_ocr_assisted_identification)
        ocr_ai_menu.add_command(label="AI Grading Assistant", command=self.open_ai_grading_assistant)
        ocr_ai_menu.add_command(label="Smart Phone Cataloguer", command=self.open_smart_phone_cataloguer)

        # Mobile & Sync
        mobile_sync_menu = tk.Menu(tools_menu, tearoff=0)
        tools_menu.add_cascade(label="Mobile & Sync", menu=mobile_sync_menu)
        mobile_sync_menu.add_command(label="Mobile Collection Entry", command=self.open_mobile_collection_entry)
        mobile_sync_menu.add_command(label="Collector Workflow Integration", command=self.open_collector_workflow_integration)
        mobile_sync_menu.add_command(label="Collector Cloud Foundation", command=self.open_collector_cloud_foundation)
        mobile_sync_menu.add_command(label="Sync & Backup", command=self.open_sync_backup)
        mobile_sync_menu.add_command(label="Multi-Device Workspace", command=self.open_multi_device_workspace)
        mobile_sync_menu.add_command(label="Device Linking & Conflict Resolution", command=self.open_device_linking)
        mobile_sync_menu.add_command(label="Mobile Collector Companion", command=self.open_mobile_collector_companion)
        mobile_sync_menu.add_command(label="Phone Photo Capture", command=self.open_phone_photo_capture)

        # Market & Deal Tools
        market_deal_menu = tk.Menu(tools_menu, tearoff=0)
        tools_menu.add_cascade(label="Market & Deal Tools", menu=market_deal_menu)
        market_deal_menu.add_command(label="Deal Hunter Ranking", command=self.open_deal_hunter_ranking)
        market_deal_menu.add_command(label="Deal Hunter Calibration", command=self.open_deal_hunter_calibration)
        market_deal_menu.add_command(label="External Listing Connectors", command=self.open_external_listing_connectors)
        market_deal_menu.add_command(label="Live Deal Hunter", command=self.open_live_deal_hunter)
        market_deal_menu.add_command(label="Live Source Validation", command=self.open_live_source_validation)
        market_deal_menu.add_command(label="Live Deal Hunter Readiness", command=self.open_live_deal_hunter_readiness)
        market_deal_menu.add_command(label="Market Intelligence", command=self.open_market_intelligence)
        market_deal_menu.add_command(label="Market Intelligence Automation", command=self.open_market_intelligence_automation)
        market_deal_menu.add_command(label="Watchlists & Alerts", command=self.open_watchlists_and_alerts)

        # Platform & Diagnostics
        platform_diagnostics_menu = tk.Menu(tools_menu, tearoff=0)
        tools_menu.add_cascade(label="Platform & Diagnostics", menu=platform_diagnostics_menu)
        platform_diagnostics_menu.add_command(label="Platform Management", command=self.open_platform_management)
        platform_diagnostics_menu.add_command(label="Platform Analytics", command=self.open_platform_analytics)
        platform_diagnostics_menu.add_command(label="Field Test & Tuning", command=self.open_field_test_and_tuning)
        platform_diagnostics_menu.add_command(label="Batch Processing", command=self.open_batch_processing)
