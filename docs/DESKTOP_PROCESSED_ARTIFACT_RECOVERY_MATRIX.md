...
| PA-RM-13 | Cancel or successful zero-selection no-op after both snapshots are prepared, before `PREPARED` | Both verified preparation handles and final decisions | Clean processed snapshot, then raw snapshot | No durable transaction state or terminal history; zero-selection returns the existing no-op result | `test_pa_rm13_prejournal_cancel_or_zero_selection_cleans_both_snapshots` |
