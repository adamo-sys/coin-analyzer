...
Use this guide to preserve the v1.0 repository, release documentation, collection data, and legacy workbook sources.
...
```powershell
git bundle create coin-analyzer-v1.0.bundle --all
```

Verify the bundle:

```powershell
git bundle verify coin-analyzer-v1.0.bundle
```

Restore from the bundle into a new folder:

```powershell
git clone coin-analyzer-v1.0.bundle coin-analyzer-restore
```
...
```powershell
Get-FileHash coin-analyzer-v1.0.bundle -Algorithm SHA256
```
...
- Keep a dated copy, such as `collection-2026-06-16-v1.0.json`.
