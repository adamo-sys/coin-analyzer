...
The public semantic-version release line is distinct from the historical internal milestone numbering preserved below.
...
| `v1.3.0-rc2` | Pending | Technically accepted application lineage: `d209d14`; final tag target pending documentation-only reconciliation | Repaired PR #288 candidate with RC2 metadata/runtime reconciliation. Technical acceptance is complete; manual acceptance, explicit tag authorization, and release remain pending. |
...
| `v8.3` | `ccc9bd0` | Collector Workspace — unified operating system aggregating every existing tool, engine, report, and workflow into a single cohesive workspace with Inbox, Collection Overview, Want List & Opportunities, AI Assistant Queue, Batch Queue, Photo Vault, Dashboard, Reports, Workflow, and Data Safety panels. Thin orchestration layer with zero duplicated intelligence, zero duplicated collection, zero ML, zero CV. |
| `v8.4` | `5691743` | Connected Data — thin `ConnectedDataEngine` cross-reference facade with `ConnectionType` enums, `MatchType` classification, 12 connection-type pair dispatch table, exact-year fuzzy matching, exact path matching, derived links. `ConnectedDataReport` integrated into `CollectorWorkspace` via `get_connected_data()`. Metadata-only enrichment in `SmartShoppingAssistant.generate_report()`. Connected Data GUI tab in workspace notebook. 1213-test regression pass. No new intelligence, no new persistence, no mutation. |
